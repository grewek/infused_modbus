...
**FC backlog: FC 20 done.** Per the confirmed ordering (2026-09-24), FC 21 (Write File Record) is next — now actually implementable since its `server-options.toml` gating prerequisite is done; still needs a technician to explicitly enable `write_file_record`. FC 18 (Read FIFO Queue) remains hardest/last.
